...
// toClientConnection is function to convert sock to client connection info
func (ms *ModbusServer) toClientConnection(sock net.Conn) *ClientConnection {
	cc := ClientConnection{}
	if sock == nil {
		return &cc
	}
	...
	if sock.LocalAddr() != nil {
		lAdrr = sock.LocalAddr().String()
	}
	if sock.RemoteAddr() != nil {
		lAdrr = sock.RemoteAddr().String()
	}
	total := len(ms.tcpClients)
	cc.LocalAddr = lAdrr
	cc.RemoteAddr = rAdrr
	cc.TotalConn = uint8(total)
	return &cc
}
...
func (ms *ModbusServer) Stop() (err error) {
	...
	if ms.transportType == modbusTCP || ms.transportType == modbusTCPOverTLS {
		...
		for _, sock := range ms.tcpClients {
			// Client Connection
			cc := ms.toClientConnection(sock)

			// Close
			sock.Close()

			// Call Handler
			ms.handler.HandleClientDisconnected(cc)
		}
	}
...
func (ms *ModbusServer) acceptTCPClients() {
	...
	for {
		...
		if ms.started && uint(len(ms.tcpClients)) < ms.conf.MaxClients {
			...
			// Call Handler
			ms.handler.HandleClientConnected(cc)
